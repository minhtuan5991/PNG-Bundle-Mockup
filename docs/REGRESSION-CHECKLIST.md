...
- [ ] Chưa kiểm thử cài mới/nâng cấp tương tác v1.4.11 trên máy hoặc VM sạch.
...
- [x] GitHub v1.4.8 đã xác minh ngày 2026-08-28: commit `56c2ba6`, Windows CI `33150064113` và Release Windows `33150223710` đều thành công; Release `378312944` stable/public/latest có đúng ba artifact tải lại khớp kích thước/SHA-256 và updater version/path/size/SHA-512.
...
## E13. v1.4.3 — đổi tên liên tiếp trong popup Group Shirt

- [x] `package.json`, `package-lock.json` và package root cùng version `1.4.3`.
- [x] Popup có đúng ba nút **Hủy**, **Đổi Tên**, **Đổi tên và Đóng**; chỉ một nút dùng style primary.
- [x] **Đổi Tên** gọi rename với `closeAfterSave=false`; **Đổi tên và Đóng** gọi với `true`.
- [x] Sau rename giữ mở, cả gallery, selection toàn app và selection trong popup dùng đường dẫn mới.
- [x] Hủy/dấu ×/Esc không gọi API ghi file; khi đang ghi, control bị khóa và có phản hồi **Đang đổi tên…**.
- [x] `node --check`, `git diff --check` và automated tests **122/122** đạt.
- [x] Electron source và packaged smoke **22/22** đạt, gồm `renameDialogActions`.
- [x] Build local: Setup 104.370.304 byte (`2E712A0E…BAA11`), blockmap 109.428 byte (`83BF9E9C…CA010`), `latest.yml` 363 byte (`7D6B250F…A5C36`); metadata khớp, Authenticode NotSigned.
- [x] `app.asar` v1.4.3 chứa đủ hai nhánh giữ/đóng; packaged Input đúng hai file allowlist và bốn JPG riêng không được đóng gói.
- [x] Commit `9f63ff7113f9cb73d8d3d750d3b9fde25077930f`/tag `v1.4.3`, Windows CI `32858769735`, Release Windows `32858769967` và Release ID `376464636` stable/public có đúng ba asset đã xác minh.

## E14. v1.4.4 — selection đổi tên tường minh và thumbnail lớn

- [x] `package.json`, `package-lock.json` và package root cùng version `1.4.4`.
- [x] Popup mở với `selected: new Set()` và `0/N`; không còn `files.map(...)` tự chọn toàn bộ PNG.
- [x] Phần đếm, validation và `filePaths` gửi IPC đều lấy từ `selectedRenameFiles()`; backend chỉ đổi các path trong payload.
- [x] Click thumbnail vẫn add/delete đúng path; thumbnail chưa chọn hiện **Không đổi tên**; chọn/bỏ chọn đang hiện giữ đúng phạm vi tìm kiếm.
- [x] Thumbnail là `80×76 px`, card tối thiểu `92 px`, grid tối thiểu `178 px`, khoảng cách `12 px` và giữ `object-fit: contain`.
- [x] `node --check`, `git diff --check` và automated tests **124/124** đạt.
...
| v1.4.9 Print Area QA/release | Codex local QA + GitHub Actions | 2026-08-30 | 159/159 test; packaged smoke 23/23; headless migration AppData → Print Area → snapshot exit 0 và SHA-256 khớp. Commit `d331844`, CI/Release workflow success; Release ID `379199422` stable/public/latest có đúng ba asset tải ngược đã xác minh. Chưa cài/nâng cấp tương tác. |
| v1.4.12 gender regions QA/release | Codex local QA + GitHub Actions | 2026-09-01 | 169/169 test; source/package smoke 25/25; schema v1/v2 migration, gender-first matching, mockup đơn không bị ảnh hưởng, ASAR/version/updater metadata/Input/Print Area allowlist đạt. Commit `2d5cbb4`, CI/Release workflow success; Release `380666786` public/stable/latest có đúng ba asset đã được workflow tải ngược xác minh. Cài tương tác còn chờ. |
