...
test('quy tắc 1: PNG không tag dùng mọi màu mặt trước nhưng không dùng nền có mặt sau', async () => {
  ...
  assert.deepEqual(templateNames(plan), ['dark mgs.png', 'light mgs.png', 'mixed mgs.png']);
  assert.ok(plan.outputs.flatMap((output) => output.assignments)
    .every((assignment) => assignment.region.side === 'front'));
  ...
  assert.ok(plan.warnings.some((warning) => warning.template.name === 'back mgs.png'));
});
