...
Sau khi cài, dùng nút **Mở Input** để xác nhận thư mục `Input` nằm cạnh EXE và thêm PDF/ảnh mẫu của bạn. Người đang dùng installer `1.2.0` sẽ nhận `1.2.1` qua GitHub updater sau khi release stable có đủ installer, blockmap và `latest.yml`; kiểm thử nâng cấp live phải hoàn tất trước khi coi bản phát hành đã chốt.
