...
- Group Shirt nhận tên `1 (1).wh.f.m.png`, `1 (2).bl.b.w.png`; tag `.m/.w` dành cho vùng Áo nam/Áo nữ của Mockup Cặp đôi. PNG không có `.m/.w` dùng được cả vùng Nam/Nữ và chỉ xét màu áo, mặt trước/sau; PNG có tag giới tính vẫn bắt buộc đúng Nam/Nữ. Mỗi PNG không có tag màu dùng cả áo sáng/tối và giữ đúng mặt, kể cả trong nhóm trộn tag. Nhóm không có PNG mặt sau tự bỏ qua toàn bộ nền có vùng mặt sau.
...
Sau khi cài, dùng nút **Mở Input** để xác nhận thư mục `Input` nằm cạnh EXE và thêm PDF/ảnh mẫu của bạn. Mã nguồn và bản stable mới nhất là [`v1.4.12`](https://github.com/minhtuan5991/PNG-Bundle-Mockup/releases/tag/v1.4.12); xem [ghi chú v1.4.12](docs/RELEASE-NOTES-1.4.12.md) và tải bộ cài từ [GitHub Releases](https://github.com/minhtuan5991/PNG-Bundle-Mockup/releases/latest).
